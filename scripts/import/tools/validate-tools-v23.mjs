...
for (const [index, row] of rows.entries()) {
  ...
  if (!VALID_TOOL_TYPES.has(toolType)) {
    errors.push(
      `Fila ${excelRow} (${toolId}): tool_type inválido: ${toolType || "(vacío)"}.`,
    );
  }
...
console.log("");
console.log("======================================");
console.log(" ELECTROTOOLS — VALIDACIÓN V23");
console.log("======================================");
console.log("");
...
console.log("ElectroTools:");

for (const row of rows) {
  console.log(
    `  ${row.tool_id} | ${row.name} | ${row.tool_type} | ${row.release} | ${row.status || "planned"}`,
  );
}
